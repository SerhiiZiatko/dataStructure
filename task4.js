...
function filterArrays(arr1, arr2){
    return arr1.concat(arr2).filter(item => (parseInt(item.balance.replace(/[$,]{0,}/g, '')) > 2000)
                                             && (item.age < 30)
                                             && (item.eyeColor == 'blue' || item.eyeColor == 'brown')
                                             && (item.tags.indexOf('velit') !== -1 && item.tags.indexOf('anim')) );
}
...
function copy(...arrays){
    return arrays.reduce((prev, item) => prev.concat(item))
}
