...
function isDate(date) { //TODO: check position of ./-//
    let is_date = true;
    let date_day =  parseInt(date.slice(0 , 2));
    let date_mounth = parseInt(date.slice(3, 5));
    let date_year = parseInt(date.slice(6, 10));

    if (isNaN(date_day) || isNaN(date_year) || isNaN(date_mounth)) {
        is_date = false;
    }
...
console.log(isDate('12-05-2020')); // true
console.log(isDate('12/05/2020')); // true
console.log(isDate('dfsdfsdf')); // falses

function isPhoneNumber(number) {
    let number_clear = number.split('-').join('');
    let is_valid = true;
...
function writew() {
    let font_width = 5,
        height = 8,
        length = 3,
        v_length = height + (height - font_width) / 2,
        row = '',
        row_revers,
        row_length = '',
        final_row = '';
    ...
    for (let i = 0; i < height; i++) {
        ...
        row_revers = row.split('').reverse().join('');
        row += row_revers;
...
function createPatternArray(str) {
  let el_mas = str.split(' '),
      parent_array = [];
  for (let i = 0; i < el_mas.length; i++) {
      let obj = {};
          obj.name = el_mas[i];
          i++;
          obj.id = el_mas[i];
...
//3.2
function findArray (arr_anyth, arr_numb) {
    let mas= [];

    for (let i = 0; i < arr_numb.length; i++ ) {
        let el = arr_anyth[arr_numb[i]];

        if (el !== undefined)
            mas.push(arr_anyth[arr_numb[i]]);
    }
...
//3.3
function avarageMasEl(mas) {
    let length = 0;
    let avarage_letter_el = '';
    let avarage_el_mas = [];
    ...
    let avarage = Math.round(length / mas.length);
    let mas_avarage_el;

    for (let i = 0; i < mas.length; i++) {
        if (mas[i].length !== avarage) {
            let el_letter = mas[i].slice(0, 1);

            for (let j = 0; j < avarage; j++) {
                avarage_letter_el += el_letter;
            }
        } else {
            avarage_letter_el = mas[i];
        }

        avarage_el_mas.push(avarage_letter_el);

        avarage_letter_el = '';
        mas_avarage_el = '';
    }

    return console.log(avarage_el_mas);
}

avarageMasEl( ['u', 'y'] );
avarageMasEl( ['aa', 'bbb', 'cccc'] );
avarageMasEl( ['aa', 'bb', 'ddd', 'eee'] );

console.log('Fouth ex');
//4.1

function newMas(arr1, arr2) {
    let concat_mas = arr1.concat(arr2);
    let new_mas = [];

    for (let i = 0; i < concat_mas.length; i++) {
        let obj = concat_mas[i];
        let obj_age = (obj['age'] < 30);
        let balance = obj['balance'].slice(1, obj['balance'].length).split(',').join('');
        let obj_balance = (balance > 2000) ;
        let obj_eye_color = (obj['eyeColor'] === 'blue' || obj['eyeColor'] === 'brown');

        let velit = obj['tags'].indexOf('velit');
        let anim = obj['tags'].indexOf('anim');

        let obj_tags = (velit !== -1 && anim !== -1);

        if (obj_age, obj_balance, obj_eye_color, obj_tags)
            new_mas.push(obj);
    }
...
function indUniqueTags(arr1, arr2) {
    let concat_mas = arr1.concat(arr2);
    let tag_mas = [];

    for (let i = 0; i < concat_mas.length; i++) {
        let obj = concat_mas[i];
        let obj_mas_tags = obj['tags'];

        for (let j = 0; j < obj_mas_tags.length; j++) {
            tag_mas.push(obj_mas_tags[j]);
        }
    }

    let obj_tag = tag_mas.reduce(function(obj, el) {
        obj[el] = (obj[el] || 0) + 1;
        return obj;
    }, {});
    ...
    for (let key in obj_tag) tag_mas.push(key);

    return console.log(tag_mas);
}

function copy(arr1, arr2) {
    let new_array1 = []; // copy of arr1
    let new_array2 = []; // copy of arr2
    let concat_mas = arr1.concat(arr2);
    ...
    for (let i = 0; i < concat_mas.length; i++) {
        ...
        for (let key in obj) {
            if (typeof(obj[key]) === 'object') {
                let object_value = Object.assign({}, obj[key]);

                new_obj[key] = object_value;
            } else {
                new_obj[key] = obj[key];
            }
        }
        ...
        if (i < arr1.length) {
            new_array1.push(new_obj);
        } else {
            new_array2.push(new_obj);
        }
    }

    // check link?
    console.log(new_array1 !== arr1, new_array2 !== arr2, new_array1[0].tags !== arr1[0].tags);

    let main_mas = new_array1.concat(new_array2);
