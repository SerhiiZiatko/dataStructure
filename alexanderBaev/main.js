...
function findArray(arr1, arr2) {
	...
	for(let i = 0; i < arr2.length; i++) {
		helpArr.push(arr1[arr2[i]]);
	}

	return helpArr;
}

function averageItemsLength(arr) {
	let equalItems = [];
...
function filterArrays(arr1, arr2) {
	let allUsers = arr1.concat(arr2);
	let filteredUsers = [];
	for (let i = 0; i < allUsers.length; i++) {

		let itemBalance = allUsers[i].balance;
		let indexComa = itemBalance.indexOf(',');

		if (itemBalance[0] == "$") {
			itemBalance = itemBalance.slice(1);
		}
		if (indexComa > 0) {
			itemBalance = itemBalance.slice(0, indexComa - 1) + itemBalance.slice(indexComa);
		}


		if (itemBalance > 2000) {

			if (allUsers[i].age < 30) {

				if (allUsers[i].eyeColor == "brown" || allUsers[i].eyeColor == "blue") {

					if (allUsers[i].tags.indexOf("velit") >= 0 && allUsers[i].tags.indexOf("anim") >= 0) {
...
function findUniqueTags(arr1, arr2) {
	let allUsers = arr1.concat(arr2);
	let obj = {};
	let countOfUniqueTags = 0;
	let uniqueTagsWithId = [];

	for(let i = 0; i < allUsers.length; i++) {

		for(let j = 0; j < allUsers[i].tags.length; j++) {
			if(allUsers[i].tags[j] in obj) continue;

			obj[allUsers[i].tags[j]] = countOfUniqueTags;
			countOfUniqueTags++;
		}
	}
	...
	for(let keys in obj) {
		...
		helpObj.id = obj[keys];
		helpObj.name = keys;
		uniqueTagsWithId.push(helpObj);
	}

	return uniqueTagsWithId;
}
...
function copyArrays() {
	...
	for(let i = 0; i < arguments.length; i++) {
		...
		for(let j = 0; j < arguments[i].length; j++) {
			globalArrays.push(JSON.parse(JSON.stringify(arguments[i][j])));
		}
...
function drawingLetter(fontWidth, height, length) {
	if(height < 7 || fontWidth < 2) {
		console.log("Не правильно заданны условия");
		return;
	}
	...
	for(let k = 0; k < height; k++) {
		...
		for(let i = 1; i <= length; i++) {
			...
			printLetter += initialGaps + stars + gaps + gaps + stars + initialGaps;
			if (i == length) {
				printLetter += "\n";
			}
