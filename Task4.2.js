...
function findUniqueTags(arr1, arr2) {
    let tagsUnique = [];
    let tagsSetArray = new Set();
    let tagsArray = [];
    let concatArray = arr1.concat(arr2);

    for (let i = 0; i < concatArray.length; i ++) {
        for (let j = 0; j < concatArray[i]["tags"].length; j++) {
            tagsSetArray.add(concatArray[i]["tags"][j]);
        }
    }

    tagsArray = Array.from(tagsSetArray);
    for (let i = 0; i < tagsArray.length; i ++) {
        tagsUnique.push({id: i, name: tagsArray[i]})
    }
