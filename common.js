...
function ucFirst( str ) {
    let res = str[0].toUpperCase() + str.substr(1);
    console.log(res)
}
// ucFirst('to upper');
...
// ucWords('some text to upper')


function round( num ){
    let res = parseFloat(num.toFixed(8));
    console.log(res)
...
function isEmail( mail ) {
    let status = true,
        posAt = mail.indexOf('@'),
        posDot = mail.indexOf('.'),
        domen = mail.slice( ++posDot);

    if ( ( posAt < 3) || (((posDot-1) - (posAt+1)) < 3) || ( domen.length < 2) ){
        status = false;
    }
...
// let phoneNum = isPhoneNumber('0380989817689');
...
function createPatternArray( string ) {
    let arr = [],
        allValue = string.split(' ');
    for (let i = 0; i < allValue.length; i++){

        if (!(i % 2)) {
            let obj = {};
            obj.color =  allValue[i];
            obj.id =  allValue[i+1];
            arr.push(obj)
        }
    }
...
function findArray( arr1, arr2  ) {
    let resArr = [],
        varify = arr2.every(item => Number.isInteger(item));
    if ((arr1.length == 0) || (arr2.length == 0)){
        console.log(resArr)
    } else if (varify){
        for (let i = 0; i < arr2.length; i++){
            resArr.push(arr1[arr2[i]]);
        }
        console.log(resArr)
    }else {
        console.log('enter integer value in arr2')
    }

}
// findArray( [], [0, 1, 5, 2, 1, 8, 9, 1, 5]);

function arrange( arr ){
    let newArr = [],
        symbAm = arr.reduce( function(sum, next){
            return sum += next.length;
        }, 0),
        averageAm = Math.round(symbAm/arr.length);


    for (let i = 0; i < arr.length; i++){
        let symb = arr[i][0],
            fullLengthStr = '';
        for (let j = 0; j < averageAm; j++){
            fullLengthStr += symb;
        }
        newArr.push(fullLengthStr)
    }

    return newArr ;
}
// let res = arrange(['aa', 'bb', 'ddd', 'eee']);
// console.log(res)
...
function filterForArrays( arr1, arr2 ) {
    let arr3 = [];


    function filter (arr){
        for( let i = 0; i < arr.length; i++ ){
            let notEdited = arr[i].balance;
            let balance = '';

            for (let i = 0; i < notEdited.length; i++){
                if ((notEdited[i] == ',') || (notEdited[i] == '$')){
                    continue
                }
                else{
                    balance += notEdited[i];
            ...
            let tags = arr[i].tags,
                tagCircumst = (tags.indexOf('velit') >= 0 && tags.indexOf('anim') >= 0)


            if ( ((arr[i].age) < 30) &&
                (balance > 2000) &&
                ( ((arr[i].eyeColor) == 'blue') || ((arr[i].eyeColor) == 'brown')) &&
                tagCircumst){
                    arr3.push(arr[i])
...
// let filter = filterForArrays(arr1, arr2);
// console.log(filter)
...
// let uniq = findUniqueTags(arr1, arr2);
// console.log(uniq)
