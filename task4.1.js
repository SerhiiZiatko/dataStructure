...
function filterArrays(arr1, arr2) {
    ...
    for(let i = 0; i < arr3.length; i++) {
        if(parsePriceToInt(arr3[i].balance) > 2000
            && arr3[i].age < 30
            && arr3[i].eyeColor == 'blue'
            || arr3[i].eyeColor == 'brown'
            && arr3[i].tags.includes('velit')
            && arr3[i].tags.includes('anim')
        ){
            filterArray.push(arr3[i]);
        }
    }
...
function parsePriceToInt(price) {
    return parseInt(price.replace(/^\$?/g, '').replace(/\,?/g, ''));
}
