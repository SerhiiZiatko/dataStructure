...
//7
function isDate(str) {
    if (new Date(str).toLowerCase() == 'invalid date') return false;
    return true;
};
...
PatternArray("red 1 yellow 2 black 3 white 4");
...
function duplicate (arr){
    ...
    average = Math.ceil(sum / arr.length);
    for (let i = 0; i<arr.length; i++) {
        let str = arr[i].slice(1, 2);
        newArr.push(new Array(average+1).join(str));
    }
    console.log(newArr);
};
...
function filterUsers(arr1, arr2) {
    let arr = arr1.concat(arr2);
    let filterUsers = [];

    for (let i=0; i < arr.length; i++) {
        let foo = arr[i].balance;
        if (foo[0] == "$") {
            foo = foo.slice(1);
        }
        if (foo.indexOf(',') >= 0) {
            foo = foo.slice(0, foo.indexOf(',')) + foo.slice(foo.indexOf(",") + 1);
        }
        if (foo > 2000) {
            if (arr[i].age < 30) {
                if (arr[i].eyeColor == "blue" || arr[i].eyeColor == "brown") {
                    if (arr[i].tags.indexOf('velit' ) >= 0 && arr[i].tags.indexOf('anim') >= 0) {
                        filterUsers.push(arr[i]);
                    }
                }
            }
        }
...
console.log(filterUsers(arr1, arr2));
...
let newArr = [];
    ...
    for (let i = 0; i < arr.length; i++) {
        ...
        for (let j = 0; j < arr[i].tags.length; j++) {
            if (tagsArr.indexOf(arr[i].tags[j] == -1) {
                let obj = {
                    name: arr[i].tags[j],
                    id: count
                };
